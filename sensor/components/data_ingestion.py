...
class DataIngestion:
    ...
    def split_data_as_train_test(self, dataframe:pd.DataFrame)-> pd.DataFrame:
        try:
            logging.info("Initiating train test split :: START")
            ...
            test_set.to_csv(self.data_ingestion_config.test_fie_path, index=False, header=True)
            logging.info("Exported data to train and test path")
        except Exception as e:
            raise SensorException(e, sys)
    ...
    def initiate_data_ingestion(self)-> DataIngestionArtifact:
        try:
            ...
            data_ingestion_artifact = DataIngestionArtifact(train_file_path = self.data_ingestion_config.train_file_path, test_file_path = self.data_ingestion_config.test_fie_path)
